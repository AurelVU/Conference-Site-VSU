...
def logout():
    """
            ...
            responses:
              200:
                description:
                  Возвращает авторизированному пользователю Главную страницу сайта.
                  Пользователь после этого действия разлогирован.
              302:
                description:
                  Если пользователь не авторизован, то редирект на страницу Авторизации.
              400:
...
def send_message():
    ...
    Для отправки сообщений используется библиотека Flask-IO. Swagger не поддерживает документирование такого типа. Для взаимодействия с данным веб-сокетом необходимо произвести действия, такие как
      1) импортируем следующие скрипты <script type="text/javascript" src="//code.jquery.com/jquery-1.4.2.min.js"></script>, <script type="text/javascript" src="//cdnjs.cloudflare.com/ajax/libs/socket.io/1.3.6/socket.."></script>
      2) Создаем переменную сокет и подключаемся var socket с помощью socket = io.connect('http://www.student-conference.site//send_message'). Если подключение успешно, то
      -отправка сообщений - socket.emit('text', {msg.. text, id_from.. user_id , id_to.. id }), где text - текст сообщения user_id - id отправителя
      id_to - id отправителя
      -socket.emit('joined', {}) - подключение к комнате
      -socket.emit('left', {}, function() {socket.disconnect().,})., - отключение от беседы
      Ответы.. При подключении/отключении: data.msg - сообщение, При приеме сообщения - data.msg сообщение, data.timestamp - время отправления, data.recipient_id - id получателя
